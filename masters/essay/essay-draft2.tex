...
Fix a commutative ring map $A\to R$. A differential operator $D \in D_{R/A}(M)$ is said to be of \emph{order $k$} if $D \in D^k_{R/A}(M)$ but $D \not\in D^{k-1}_{R/A}(M)$ and we say $\ord(D) = k$. As the operator $0$ is contained in $D^k_{R/A}$ for every $k \in \bZ$, we say $\ord(0) = -\infty$. We already understand operators of order 0 -- they're simply the $R$-linear endomorphisms of $M$ -- and in this section we seek to understand the operators of order 1 as well. We additionally describe how order interacts with composition, addition, and commutation.

Throughout this section $A\to R$ is a map of commutative rings and $M$ is an $R$-module.
...
Now let us focus on the $R$-module $D^1_{R/A}(M)$.
