...
\end{example}
\begin{example}\label{example:cusp}
...
We would now like to calculate explicit examples of $D_{R/A}$ for rings other than polynomial rings. 

\subsection{Sheaf of Differential Operators Over a Smooth Variety}
...
\begin{thm}\label{thm}
	...
	\begin{align*}
		...
		\end{cases}.
	\end{align*}
	Moreover, we can choose the functions $x_1,...,x_n$ so that they generate the maximal ideal $\frakm_p$ of $\cO_{X,p}$.
\end{thm}
\begin{prf}
