...
Alternatively, one can define $D_X$ by gluing on open affines. One does this by setting $\Gamma(U,D_X) = D_{\Gamma(U,\cO_X)/K}$ for each open affine $U \subseteq X$. For this to work, we need the following compatibility result:
...
As should be expected by now, many algebraic constructions are brushed under the rug. The now familiar resources \cite{ginzburg_d-mod} serves as a good reference. Throughout this section, $K$ is a field of characteristic zero and both $X$ and $Y$ are smooth algebraic varieties over $K$.
...
\begin{example}[Loss of Coherence]\label{example:badly-behaved-inv-img}
...
\begin{prf}
	...
	\begin{align*}
		D_{X\to Y} 
		  ...
		  &\cong \cO_X\otimes_{\iota^{-1}\cO_Y} \iota^{-1}\left(D^X_Y\otimes_K K[\partial_{r+1},...,\partial_{n}]\right) \\
		  &\cong \left(\cO_X\otimes_{\iota^{-1}\cO_Y} \iota^{-1}D^X_Y\right)\otimes_K K[\partial_{r+1},...,\partial_{n}] \\
		  &\cong D_X\otimes_K K[\partial_{r+1},...,\partial_{n}],
	\end{align*}
...
\begin{defn}\label{defn:direct-image-right}
	Let $\cM$ be a right $D_X$-module. The pushforward of $\cM$ along $\iota$ is
	\begin{align*}
		\iota_+(M) = \iota_*(M\otimes_{D_X} D_{X\to Y}),
	\end{align*}
	which is a right $D_Y$-module under the morphism $D_Y\to \iota_*\iota^{-1}D_Y$. 
\end{defn}

\end{defn}

\subsection{Direct Images in the Affine Case}
