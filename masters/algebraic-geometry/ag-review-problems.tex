...
\begin{homework}[e]
	...
	\begin{prf}
		The obvious example is $\bZ\hookrightarrow \bQ$ or $R \hookrightarrow \Frac(R)$ for a domain $R$. Pulling back the ideal $(0) \subseteq \Frac(R)$ gives back $(0)$, which is not maximal in $R$ unless $R$ is also a field.

		The actual lesson to learn here is that localizing at a prime turns that prime into a maximal ideal -- take for instance $k[x,y,z] \hookrightarrow k[x,y,z]_{(x)}$ (here we're inverting everything outside $(x)$, we're not inverting $x$ itself). Pulling back $(x)$, maximal in $k[x,y,z]_{(x)}$, gives back $(x) \subseteq k[x,y,z]$, not maximal.

		We now prove the second part. Suppose $\varphi:R\to S$ is a surjective ring homomorphism and $\frakm \subseteq S$ is maximal. Then $\ol{\varphi}:R\to{\varphi}S\to S/\frakm$ is a surjective map from $R$ to a field, meaning $\ker \frakm$ is maximal since $R/\ker \ol{\varphi}$ is a field. However, $r \in \ker\ol{\varphi}$ if and only if $\varphi(r) \in \frakm$, meaning $\ker\ol{\varphi} = \varphi^{-1}(\frakm)$. Thus $\varphi^{-1}(\frakm)$ is maximal in $R$.
	\end{prf}
	...
	\begin{prf}
		The ring is $A_1\times A_2$. The primes in this ring are exactly those of the form $\frakp \times A_2$ or $A_1\times \frakq$ for $\frakp \in X_1$ and $\frakq \in X_2$.
	\end{prf}
	...
	\begin{prf}
		Specifying a morphism $(f,f^{sh}):X\to \Spec \bZ$ requires specifying $f:X\to \Spec\bZ$ and $f^{sh}:\cO_\bZ \to f_*\cO_X$. Let's examine $f$ first.

		Suppose $U\subseteq X$ is affine. We then have a map $\varphi_U:U\to \Spec \bZ$ determined by the unique ring map $\varphi_U^*:\bZ \to \cO_X(U)$. Suppose now that we are given two affine opens $U,V \subseteq X$. Let $\iota_U:U\cap V \to U$ and $\iota_V:U\cap V\to V$ denote the inclusion of $U\cap V$ into $U$ and $V$ respectively. By the uniqueness of
	\end{prf}
